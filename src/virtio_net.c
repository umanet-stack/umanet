...
void vs_vhost_net_setup(struct vhost_dev *dev) {
    ...
    RTE_LOG(INFO, VHOST_CONFIG, "setting builtin vhost-user net driver\n");
...
void vs_vhost_net_remove(struct vhost_dev *dev) { free(dev->mem); }
...
static __rte_always_inline int enqueue_pkt(struct vhost_dev *dev,
    ...
    if (likely(desc_chunck_len >= dev->hdr_len)) { // Fast path: header fits in the first contiguous chunk
        *(struct virtio_net_hdr *)(uintptr_t)desc_addr = virtio_hdr;
        ...
        while (remain) {
            ...
            dst = rte_vhost_va_from_guest_pa(dev->mem, guest_addr, &len);
            if (unlikely(!dst || !len))
                return -1;
        ...
        desc_addr = rte_vhost_va_from_guest_pa(dev->mem, desc_gaddr, &desc_chunck_len);
    ...
    while (mbuf_avail != 0 || m->next != NULL) {
        ...
        if (desc_avail == 0) {
            ...
            if (unlikely(desc->next >= vr->size || ++nr_desc > vr->size))
                return -1;
        ...
        rte_memcpy((void *)((uintptr_t)(desc_addr + desc_offset)), rte_pktmbuf_mtod_offset(m, void *, mbuf_offset),
                   cpy_len);
...
uint16_t vs_enqueue_pkts(struct vhost_dev *dev, uint16_t queue_id, struct rte_mbuf **pkts, uint32_t count) {
    ...
    for (i = 0; i < count; i++) {
        ...
        vr->used->ring[used_idx].len = pkts[i]->pkt_len + dev->hdr_len;
    }
...
static __rte_always_inline int dequeue_pkt(struct vhost_dev *dev, struct rte_vhost_vring *vr, struct rte_mbuf *m,
    ...
    desc = &vr->desc[desc_idx];
    if (unlikely((desc->len < dev->hdr_len)) || (desc->flags & VRING_DESC_F_INDIRECT))
        return -1;
    ...
    while (1) {
        cpy_len = RTE_MIN(desc_chunck_len, mbuf_avail);
        rte_memcpy(rte_pktmbuf_mtod_offset(cur, void *, mbuf_offset), (void *)((uintptr_t)(desc_addr + desc_offset)),
                   cpy_len);
...
uint16_t vs_dequeue_pkts(struct vhost_dev *dev, uint16_t queue_id, struct rte_mempool *mbuf_pool,
    ...
    free_entries = *((volatile uint16_t *)&vr->avail->idx) - queue->last_avail_idx;
    if (free_entries == 0)
        return 0;
