...
//  static void vhost_tx(struct vhost_dev *dst_vdev, struct vhost_dev *src_vdev, struct rte_mbuf **pkts, uint16_t
//  count); static void route_vhost_pkts(struct dataplane_context *ctx, struct rte_mbuf **pkts, uint16_t *pkts_count,
//                               uint16_t current_device_num);
...
//      route_vhost_pkts(ctx, pkts, pkts_count, current_device_num);

//      return packets_received;
//  }

//  static void route_vhost_pkts(struct dataplane_context *ctx, struct rte_mbuf **pkts, uint16_t *pkts_count,
//                               uint16_t current_device_num) {
...
//          struct rte_ether_hdr *eth_hdr = rte_pktmbuf_mtod(pkts[i * MAX_PKT_BURST], struct rte_ether_hdr *);
//          vdev = find_vhost_dev(&eth_hdr->d_addr);
//          if (unlikely(vdev == NULL)) {
...
//                  if (external_pkts[external_count]->ol_flags & PKT_TX_TCP_SEG) // if TCP segmentation offload is
//                  enabled
...
//          if (local_counts[i] == 0)
//              continue;
//          struct rte_ether_hdr *eth_hdr = rte_pktmbuf_mtod(local_pkts[i * MAX_PKT_BURST], struct rte_ether_hdr *);
//          vdev = find_vhost_dev(&eth_hdr->d_addr);
//          if (unlikely(vdev == NULL)) {
//              LOG_WARN("Warning: NULL vdev at index %d in route_vhost_local\n", i);
//              continue;
//          }
//          route_vhost_local(vdev, local_pkts + i * MAX_PKT_BURST, local_counts[i]);
//      }
//  }
